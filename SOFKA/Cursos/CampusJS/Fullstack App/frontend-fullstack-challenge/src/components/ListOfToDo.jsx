...
const ListOfToDo = () => {
  ...
  const fetchAllLists = async () => {
    let response = await fetch(`http://localhost:8081/api/get/titles`)
  ...
  const onAddTask = async (fk, event) => {
    ...
    if (task) {
      ...
      let taskSavedPromise = await fetch(`http://localhost:8081/api/create/tasks`,
        {
          method: 'POST',
          headers: {
  ...
  const onCheckBox = async (event, tasks) => {
    ...
    let taskSwitchedPromise = await fetch(`http://localhost:8081/api/update/task`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
  ...
  const onDeleteTask = async (tasks) => {
    let response = await fetch(`http://localhost:8081/api/delete/task/${tasks.id}`,
    {
      method: 'DELETE'
    })
  ...
  const onDeleteTitle = async(title) => {
    let response = await fetch(`http://localhost:8081/api/delete/title/${title.id}`,
    {
      method: 'DELETE'
    })
          ...
          return <div key={title.id}>
            ...
            <form>
              <div style={{ display: visible ? 'none' : 'block' }}>
                <input onChange={addingTask} type="text" name={title.id} placeholder="Type new task" />
                <button onClick={e => { onAddTask(title.id, e) }} name={title.id}>Set Task</button>
              </div>
